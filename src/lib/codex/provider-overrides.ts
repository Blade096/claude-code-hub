...
function applyInputImageGenerationToolPreference(
  ...
  if (enabled !== false || !Array.isArray(request.input)) return;
  ...
  if (changed) ensureCloned().input = nextInput;
}

function isImageGenerationToolChoice(value: unknown): boolean {
  if (typeof value === "string") return value === "image_generation";
  if (!isPlainObject(value)) return false;
  if (isImageGenerationTool(value)) return true;
...
function summarizeImageGenerationToolChoice(value: unknown): string | null {
  ...
  if (!isPlainObject(value)) return null;
  if (isImageGenerationTool(value.tool)) return "tool:image_generation";
